...
2. **Run all rules** against the config (with `model` now resolved). Rules can assume `config.model` is non-undefined; other fields may still be `undefined`, which is correct — rules check explicit conflicts (e.g., "user set duration=6 AND resolution=1080p"), not defaults.
